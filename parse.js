function parse(inp) {
    ...
    for (i = 0; i < lines.length; i++) {
        l = lines[i];
        l = l.trim().replace(/\(DINKEL PUNKT BEFREIUNGS ZEICHEN\)/g, ".");
        if (!l.startsWith("setze") && hasOpen) {
            hasOpen = false;
            html += ">";
        }
        if (l.startsWith("beginne")) {
            splits = l.split(" ")
            keyword = splits[splits.length - 1]
            html += "<" + keyword
            hasOpen = true
        } else if (l.startsWith("beende")) {
            splits = l.split(" ")
            keyword = splits[splits.length - 1]
            html += "</" + keyword + ">"
        } else if (l.startsWith("literarisch")) {
            html += l.split(" ").slice(1).join(" ")
        } else if (l.startsWith("setze")) {
            if (!hasOpen) {
                alert("Achtung! Achtung! Invalides Dinkel auf dieser Seite");
            }
            words = l.split(" ");
            keyword = words[1];
            value = words.slice(3).join(" ");
            html += " " + keyword + "=\"" + value + "\""
        }
    }
    return html;
...
var DINKEL = document.getElementsByTagName("dinkelkasten")[0].innerHTML;

document.documentElement.innerHTML = parse(DINKEL);
...
setInterval(() => {
    ...
    100, 30, 100, 30, 300, 30, 100, 300,
    100, 30, 300, 30, 100, 300,
    100, 300,
    100, 30, 100, 300,
    100, 30, 100, 30, 100, 30, 100, 300,
    100, 300,
    100, 30, 100, 300,
    300
    ]);
